...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # ramp panel
        rf = tk.LabelFrame(self, text=" Temperature Ramp ", font=("Segoe UI", 9),
                           bg=BG, fg=DIM, bd=1, relief="groove")
        rf.pack(fill="x", padx=12, pady=4)

        def ramp_entry(parent, label, row):
            tk.Label(parent, text=label, font=("Segoe UI", 9), bg=BG, fg=FG
                     ).grid(row=row, column=0, padx=8, pady=4, sticky="w")
            e = tk.Entry(parent, width=7, font=("Segoe UI", 9),
                         bg=CARD, fg=FG, insertbackground=FG, relief="flat", bd=4)
            e.grid(row=row, column=1, padx=4, pady=4, sticky="w")
            return e

        self.ramp_start  = ramp_entry(rf, "Start (°C):", 0)
        self.ramp_end    = ramp_entry(rf, "End (°C):",   1)
        self.ramp_time   = ramp_entry(rf, "Total time (hrs):", 2)
        self.ramp_cycles = ramp_entry(rf, "Cycles:", 3)
        self.ramp_cycles.insert(0, "1")

        self.lbl_ramp = tk.Label(rf, text="", font=("Segoe UI", 9), bg=BG, fg=ACC)
        self.lbl_ramp.grid(row=4, column=0, columnspan=3, padx=8, pady=(0, 4), sticky="w")

        ramp_btns = tk.Frame(rf, bg=BG)
        ramp_btns.grid(row=0, column=2, rowspan=4, padx=8)
        tk.Button(ramp_btns, text="Start Ramp", font=("Segoe UI", 9, "bold"),
                  bg=ACC, fg="#1e1e2e", relief="flat", padx=10, pady=4,
                  command=self._start_ramp).pack(pady=4)
        tk.Button(ramp_btns, text="Stop Ramp", font=("Segoe UI", 9, "bold"),
                  bg="#45475a", fg=FG, relief="flat", padx=10, pady=4,
                  command=self._stop_ramp).pack(pady=4)
    ...
    def _start_ramp(self):
        if not self._ready(): return
        try:
            t_start   = float(self.ramp_start.get().strip())
            t_end     = float(self.ramp_end.get().strip())
            total_hrs = float(self.ramp_time.get().strip())
            cycles    = int(self.ramp_cycles.get().strip())
        except ValueError:
            self._msg("Ramp: fill in all fields correctly."); return
        if total_hrs <= 0: self._msg("Ramp: total time must be > 0."); return
        if cycles < 1:     self._msg("Ramp: cycles must be >= 1."); return

        leg_minutes = (total_hrs * 60) / cycles
        rate = abs(t_end - t_start) / leg_minutes  # °C per minute
        direction = 1 if t_end > t_start else -1
        ...
        def run():
            try:
                with self._lock:
                    self.julabo.start()
                    self.julabo.set_setpoint(t_start)
                self.after(0, lambda: self._msg(
                    f"Ramp: {t_start}→{t_end} °C, {total_hrs}h, {cycles} cycle(s) ({leg_minutes:.1f} min each)"))
            except Exception as e:
                self.after(0, lambda m=str(e): self._msg(f"Ramp error: {m}"))
                self._ramp_running = False
                return

            # Wait until actual temp reaches start temp
            self.after(0, lambda: self.lbl_ramp.config(text=f"Waiting to reach {t_start:.2f} °C…"))
            while self._ramp_running:
                time.sleep(10)
                try:
                    with self._lock:
                        actual = self.julabo.get_temperature()
                    if actual is not None and abs(actual - t_start) <= 0.5:
                        break
                    self.after(0, lambda a=actual: self.lbl_ramp.config(
                        text=f"Reaching start temp… {a:.2f} °C"))
                except Exception:
                    pass
            ...
            for cycle in range(cycles):
                current = t_start
                with self._lock:
                    self.julabo.set_setpoint(current)
                label = f"Cycle {cycle+1}/{cycles}"
                while self._ramp_running:
                    time.sleep(60)
                    if not self._ramp_running:
                        break
                    current = round(current + rate * direction, 2)
                    if direction > 0 and current >= t_end:
                        current = t_end
                    elif direction < 0 and current <= t_end:
                        current = t_end
                    with self._lock:
                        self.julabo.set_setpoint(current)
                    self.after(0, lambda sp=current, lb=label: self.lbl_ramp.config(
                        text=f"{lb}  SP → {sp:.2f} °C"))
                    if current == t_end:
                        break
                if not self._ramp_running:
                    break
            ...
            if self._ramp_running:
                with self._lock:
                    self.julabo.set_setpoint(t_end)
                self.after(0, lambda: self.lbl_ramp.config(text=f"Holding at {t_end:.2f} °C"))
                self.after(0, lambda: self._msg(f"Ramp complete — holding at {t_end:.2f} °C"))
            self._ramp_running = False
    ...
    def _stop_ramp(self):
        self._ramp_running = False
        self.lbl_ramp.config(text="Ramp stopped.")
        self._msg("Ramp stopped.")
